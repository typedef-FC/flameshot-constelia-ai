...
void GeneralConf::_updateComponents(bool allowEmptySavePath)
{
    ...
    m_ExpireTime->setValue(config.undoLimit());
...
{
    auto* box = new QGroupBox(tr("constelia.ai license key (ABCD-EFGH-IJKL-MNOP)"));
    box->setFlat(true);
    m_layout->addWidget(box);
...
void GeneralConf::initExpireTime()
{
    auto* box = new QGroupBox(tr("constelia.ai upload expire time"));
    box->setFlat(true);
    m_layout->addWidget(box);
    ...
    m_ExpireTime = new QSpinBox(this);
    m_ExpireTime->setMinimum(0);
    m_ExpireTime->setMaximum(10000000);

    connect(m_ExpireTime,
            static_cast<void (QSpinBox::*)(int)>(&QSpinBox::valueChanged),
            this,
            &GeneralConf::expireTime);

    vboxLayout->addWidget(m_ExpireTime);
}

void GeneralConf::expireTime( int amount )
{
    ConfigHandler().setUndoLimit( amount );
}
...
void GeneralConf::initUndoLimit()
{
    ...
    m_ExpireTime = new QSpinBox(this);
    m_ExpireTime->setMinimum(1);
    m_ExpireTime->setMaximum(999);
    QString foreground = this->palette().windowText().color().name();
    m_ExpireTime->setStyleSheet(QStringLiteral("color: %1").arg(foreground));
